# ============================================================
# SEM Image Denoising — Log + N2V with Parametric GMM + BIC, Multi-Image
# ============================================================
#
# Extends denoise_N2V_GMM_bic_multi.py with a log1p pre-transform.
#
# Rationale:
...
#   log1p converts the Gamma multiplicative component to approximately additive
#   noise in the log domain. The parametric GMM then models the residual
#   signal-dependent Poisson structure remaining after log-stabilization.
#   Pooling log-domain pixel pairs from ALL training images for BIC gives
#   more statistical support, especially for low-ENL SEM.
#   Assumes homogeneous noise statistics across images (same SEM conditions).
#
# Pipeline:
#   For each training image:
#     load_sem_image() -> apply_log_transform()
#   Pool all log images:
#     -> BIC component selection      [on log-domain pooled pixel pairs]
#     -> GMM pre-training             [on log-domain (s_proxy, y_log) pairs]
#     -> joint N2V + GMM training     [log domain, multi-image]
#   For each inference image:
#     load_sem_image() -> apply_log_transform()
#       -> predict_tiled()            [log domain -> denoised_log]
#     -> inverse_log_transform()      [expm1 -> linear [0,1]]
#       -> restore original range -> save TIF / PNG
#
# Position in script family:
# ──────────────────────────────────────────────────────────────────────────────
# Script                               │ Domain │ Noise model   │ Output  │ Infer
# ─────────────────────────────────────┼────────┼───────────────┼─────────┼──────
# denoise_N2V_GMM_bic_multi.py         │ linear │ GMM + BIC     │ scalar  │ UNet
# denoise_log_N2V_GMM_bic_multi.py  <- │ log    │ GMM + BIC     │ scalar  │ UNet
# denoise_log_PPN2V_juglab_multi.py    │ log    │ GMM (manual)  │ K=800   │ MMSE
# denoise_PPN2V_juglab_bic_multi.py    │ linear │ GMM + BIC     │ K=800   │ MMSE
#
# When to use this script:
#   - Multiple speckle-dominant + Poisson mixed SEM images (ENL < 3)
#   - Want BIC auto-selection without manual trial
#   - When denoise_N2V_GMM_bic_multi.py leaves residual speckle structure
#   - Faster than log_PPN2V_juglab_multi (no bootstrap, no K=800 MMSE)
#
# References:
...
# Usage:
#   python denoise_log_N2V_GMM_bic_multi.py --input_dir ./sem_images --output_dir ./denoised
#   python denoise_log_N2V_GMM_bic_multi.py --input_dir ./sem_images --output_dir ./denoised \
#                                            --gmm_candidates 2 3 5
#   python denoise_log_N2V_GMM_bic_multi.py --input_dir ./sem_images --output_dir ./denoised \
#                                            --n_gaussians 3   # skip BIC
# ============================================================
...
def apply_log_transform(image: np.ndarray) -> Tuple[np.ndarray, float, float]:
    ...
    Steps: log1p(image) -> renormalize to [0,1].
    Returns (log_norm, log_min, log_max) for inversion.
    """
...
def inverse_log_transform(
    ...
    """Reverse normalized log domain back to linear [0,1].

    Steps: denormalize -> expm1.
    """
...
class GMMNoiseModel(nn.Module):
    """Signal-dependent GMM: p(y|s). mu_k(s)=s+offset_k, var_k(s)=exp(a_k*s+b_k)."""
    ...
    @torch.no_grad()
    def plot_noise_model(self, save_path: str = "data/noise_model_log_bic_multi.png") -> None:
        ...
        axes[0].set_xlabel("Signal s"); axes[0].set_ylabel("Noise std (log domain)")
        axes[0].set_title("Learned noise std vs. signal (log domain)"); axes[0].legend(); axes[0].grid(True, alpha=0.3)
...
# ============================================================
# 5. BIC-Based GMM Component Count Selection (multi-image, log domain)
# ============================================================
...
def select_num_gaussians_bic_multi(
    ...
    BIC = n_params * ln(n) - 2 * ln(L_hat)

    Operates on log-domain images (passed in as `images`).
    Pooling improves BIC reliability: broader signal range, lower variance.
    """
    try:
    ...
    print(f"\nBIC model selection (log domain) -- evaluating n_components in {candidates} "
...
def pretrain_gmm_multi(
    ...
    print(f"Pre-training GMM (n_components={noise_model.n_gaussians}, {n_epochs} epochs, "
          f"{N:,} log-domain pairs from {len(images)} image(s)) ...")
...
def save_outputs(
    ...
    axes[2].imshow(diff,     cmap='hot');  axes[2].set_title('Difference (x3)');             axes[2].axis('off')
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Log + N2V-GMM-BIC multi-image: log1p pre-transform + shared GMM noise model "
            "+ BIC auto-selection of n_components across a directory of SEM images."
        )
    )
    parser.add_argument('--input_dir',            type=str,   default='.')
    parser.add_argument('--train_dir',            type=str,   default='',
                        help='Optional: separate training directory (default: same as input_dir)')
    ...
    parser.add_argument('--n_gaussians',          type=int,   default=0,
                        help='Fix GMM components (skip BIC). 0 = auto via BIC (default).')
    ...
    # -- 2. Load + log-transform training images ----------------------------------
    print("\nLoading and log-transforming training images...")
    train_linear, train_meta, train_log = [], [], []
    for p in train_paths:
        img, img_min, img_max = load_sem_image(str(p))
        log_img, log_min, log_max = apply_log_transform(img)
        train_linear.append(img)
        train_meta.append((img_min, img_max, log_min, log_max))
        train_log.append(log_img)
        print(f"  {p.name}: shape={img.shape}  linear=[{img_min:.1f}, {img_max:.1f}]  "
              f"log=[{log_min:.4f}, {log_max:.4f}]")

    # -- 3. Low-count diagnostic (on linear images before transform) --------------
    print()
    for p, img in zip(train_paths, train_linear):
        bg_mean = float(img[:30, :30].mean())
        if bg_mean < 0.02:
            print(f"  WARNING [{p.name}]: background mean={bg_mean:.4f} < 0.02 "
                  "(extreme low-dose). Log transform may not help; "
                  "consider denoise_N2V_GMM_bic_multi.py instead.")
        else:
    ...
    if args.load_model and os.path.isfile(args.load_model):
        ...
        # -- 5. BIC on log-domain pixel pairs -------------------------------------
        if args.n_gaussians > 0:
            n_gaussians = args.n_gaussians
            print(f"\nUsing fixed n_components={n_gaussians} (BIC skipped)")
        else:
        ...
        # -- 6. GMM pre-training on log-domain pairs ------------------------------
        pretrain_gmm_multi(noise_model, train_log,
                           n_epochs=args.gmm_pretrain_epochs, device=device)

        # -- 7. Joint training on log-domain images -------------------------------
        train_pn2v_multi(model, noise_model, train_log,
    ...
    # -- 8. Prepare output dir + noise model plot ---------------------------------
    out_dir = Path(args.output_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    noise_model.plot_noise_model(save_path=str(out_dir / "noise_model_log_N2V_GMM_bic_multi.png"))

    # -- 9. Prepare inference images ----------------------------------------------
    if args.train_dir:
        print("\nLoading and log-transforming inference images...")
        infer_data = []
        for p in infer_paths:
            img, img_min, img_max = load_sem_image(str(p))
            log_img, log_min, log_max = apply_log_transform(img)
            infer_data.append((p, img, img_min, img_max, log_img, log_min, log_max))
            print(f"  {p.name}: shape={img.shape}")
    else:
        infer_data = []
        for p, img, (img_min, img_max, log_min, log_max), log_img in zip(
                infer_paths, train_linear, train_meta, train_log):
            infer_data.append((p, img, img_min, img_max, log_img, log_min, log_max))
    ...
    print(f"\nRunning inference on {len(infer_data)} image(s)"
          + (" (MMSE grid - experimental)..." if _nm is not None else "..."))

    for i, (p, img, img_min, img_max, log_img, log_min, log_max) in enumerate(infer_data):
        print(f"\n[{i+1}/{len(infer_data)}] {p.name}")
        denoised_log = predict_tiled(model, log_img, noise_model=_nm,
                                     tile_size=tile_size, tile_overlap=tile_overlap,
                                     device=device)
        denoised_linear = inverse_log_transform(denoised_log, log_min, log_max)
        print(f"  Inverse log transform: linear range "
              f"[{denoised_linear.min():.4f}, {denoised_linear.max():.4f}]")
        tif_path = str(out_dir / f"{p.stem}_denoised_log_N2V_GMM_bic.tif")
        png_path = str(out_dir / f"{p.stem}_comparison_log_N2V_GMM_bic.png")
        save_outputs(img, denoised_linear, img_min, img_max, tif_path, png_path)

    print(f"\nDone. n_components={n_gaussians}. All results saved to '{out_dir}/'")
