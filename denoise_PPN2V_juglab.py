# ============================================================
# SEM Image Denoising — Fully Unsupervised PPN2V (juglab-faithful)
# ============================================================
# Based on: Krull et al., "Fully Unsupervised Probabilistic Noise2Void" (2020)
#   arXiv:1911.12291
#   GitHub: github.com/juglab/pn2v
#
# Difference from denoise_PN2V_juglab.py (PN2V histogram variant):
# ────────────────────────────────────────────────────────────────
# Aspect           │ PN2V (juglab)                  │ This script (PPN2V)
# ─────────────────┼───────────────────────────────┼──────────────────────────────
# Noise model      │ Non-parametric 2D histogram    │ Parametric signal-dependent GMM
#                  │ 256×256 bins, p(y|s) per row   │ p(y|s)=Σ α_k(s)·N(μ_k(s),σ²_k(s))
# ─────────────────┼───────────────────────────────┼──────────────────────────────
# Calibration      │ Self-calibration via 4-nbr     │ Bootstrap: lightweight N2V pass
#                  │ mean proxy, or --calib_dir     │ → pseudo-clean → fit GMM → freeze;
#                  │                                │ OR --calib_dir for external images
# ─────────────────┼───────────────────────────────┼──────────────────────────────
# Calibration data │ 4-neighbor mean (crude proxy)  │ N2V pseudo-clean (lower residual)
#                  │ residual noise ≈ σ²/4          │ residual << σ² after N2V epochs
# ─────────────────┼───────────────────────────────┼──────────────────────────────
# Noise model size │ 256×256 float array (256 KB)   │ 5×n_components floats (<<1 KB)
# ─────────────────┼───────────────────────────────┼──────────────────────────────
# Fully unsupervised│ Partially (needs 4-nbr proxy) │ Yes — no external calibration
#
# UNet output: K=800 samples per pixel (same as PN2V juglab)
# Loss:        -log(1/K Σ_k p_GMM(y|s_k))  (same as PN2V juglab)
# Inference:   MMSE Σ_k p_GMM(y|s_k)·s_k / Σ_k p_GMM(y|s_k)  (same as PN2V juglab)
#
# Two-phase pipeline (bootstrap, default):
#   Phase 1: Run lightweight N2V for --n2v_epochs → pseudo-clean image
#            → Collect (pseudo_clean, noisy) pixel pairs
...
#   (same proxy as PN2V juglab self-calibration, but GMM instead of histogram).
#
# Memory note (K=800):
#   Training: (B, K, P, P) UNet output per batch.
#   B=32, K=800, P=64 → ~840 MB GPU. Reduce --batch_size if OOM.
#
# Requirements: torch>=2.0.0  tifffile  matplotlib  numpy
# Usage:
#   python test_sem.py                               # generate synthetic test image
#   python denoise_PPN2V_juglab.py                   # bootstrap mode (default)
#   python denoise_PPN2V_juglab.py --calib_dir ./sem_images
#   python denoise_PPN2V_juglab.py --n_components 5 --n2v_epochs 80
# ============================================================
...
class GMMNoiseModel(nn.Module):
    ...
    def plot(self, save_path: str = "data/gmm_noise_model.png") -> None:
        ...
        # Mixture weights: (n_pts, C)
        log_w_all = F.log_softmax(
            s_t.unsqueeze(-1) * self.weight_a.cpu() + self.weight_b.cpu(),
            dim=-1,
        ).exp().numpy()

        # Noise std and mean bias per signal level
        offsets  = self.mean_offsets.cpu().numpy()               # (C,)
        var_a    = self.var_a.cpu().numpy()
        var_b    = self.var_b.cpu().numpy()
        var_mat  = np.exp(np.outer(s_vals, var_a) + var_b[None, :])  # (n_pts, C)
        alpha    = log_w_all
...
# ============================================================
# 5. N2V Dataset — blind-spot masking
# ============================================================
...
# ============================================================
# 8. PPN2V Training — Phase 2 (negative log-evidence)
# ============================================================
...
def train_ppn2v(
    ...
    # Only UNet parameters — GMM has requires_grad=False so it is excluded automatically
...
# ============================================================
# 9. Tiled Inference — MMSE Posterior Mean
# ============================================================
...
def save_outputs(
    ...
    img_min:    float,
    img_max:    float,
    tif_path:   str = "data/denoised_sem_ppn2v_juglab.tif",
    prior_path: str = "data/denoised_sem_ppn2v_juglab_prior.tif",
    png_path:   str = "data/denoised_sem_ppn2v_juglab_comparison.png",
) -> None:
    rng        = img_max - img_min
    ...
    axes[2].imshow(mmse,  cmap='gray');  axes[2].set_title('MMSE (juglab PPN2V)');   axes[2].axis('off')
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "PPN2V juglab-faithful: parametric GMM noise model (signal-dependent) + "
            "bootstrap N2V calibration + K=800 sample UNet + MMSE posterior inference."
        )
    )
    parser.add_argument('--input',        type=str,   default='data/test_sem.tif',
                        help='Path to input .tif/.tiff/.png image')
    parser.add_argument('--output',       type=str,   default='',
                        help='Output .tif path (default: data/denoised_sem_ppn2v_juglab.tif)')
    ...
    output_path = args.output or 'data/denoised_sem_ppn2v_juglab.tif'
    ...
    if args.calib_dir:
        # Calibrated mode: use external images with 4-neighbor proxy
        ...
        # Bootstrap mode: run N2V first, use pseudo-clean as signal proxy
    ...
    noise_model.plot(save_path='data/gmm_noise_model.png')
