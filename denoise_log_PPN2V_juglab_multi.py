# ============================================================
# SEM Image Denoising — Log + PPN2V, Multi-Image (juglab-faithful)
# ============================================================
# Based on: Krull et al., "Fully Unsupervised Probabilistic Noise2Void" (2020)
#   arXiv:1911.12291
#   GitHub: github.com/juglab/pn2v
#
# Extends denoise_log_PPN2V_juglab.py to train ONE shared model on multiple images
...
#   log1p stabilises the Gamma multiplicative component; PPN2V's GMM then
#   models the residual Poisson signal-dependency in the log domain.
#
# Key design:
#   Pre-processing  │ Per-image log1p + renorm to [0,1]; log_min/log_max stored per image
#   Noise model     │ Parametric signal-dependent GMM (n_components, default 3)
#                   │ fitted from pooled log-domain (pseudo_clean, noisy) pairs
#   UNet output     │ K=800 samples per pixel
#   Training        │ Negative log-evidence: -log(1/K Σ_k p_GMM(y|s_k))
#   Inference       │ Per-image MMSE → expm1 inverse → restore original range
#
# When NOT to use:
#   Images acquired under substantially different SEM conditions. A shared GMM
#   averages noise statistics incorrectly across conditions — use
#   denoise_log_PPN2V_juglab.py per image instead.
#
# CLI:
#   python denoise_log_PPN2V_juglab_multi.py --input_dir ./sem_images --output_dir ./denoised
#   python denoise_log_PPN2V_juglab_multi.py --train_dir ./ref_imgs --input_dir ./targets --output_dir ./out
#   python denoise_log_PPN2V_juglab_multi.py --input_dir ./sem_images --save_model log_ppn2v.pt
#   python denoise_log_PPN2V_juglab_multi.py --input_dir ./new_imgs   --load_model log_ppn2v.pt
#
...
class GMMNoiseModel(nn.Module):
    ...
    In this script all pairs are in the log domain.
    """
    ...
    @torch.no_grad()
    def plot(self, save_path: str = "data/gmm_log_ppn2v_multi.png") -> None:
        C      = self.n_components
        n_pts  = 200
...
def run_bootstrap_n2v_multi(
    ...
    """
    Train one shared lightweight N2V on all log-domain images for n2v_epochs.
    Returns a list of pseudo-clean images (one per input image, log domain).
    """
    if device is None:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    print(f"Bootstrap N2V: training shared model on {n_images} image(s) "
          f"for {n2v_epochs} epochs (log domain) ...")
...
def fit_gmm_from_pairs(
    ...
    Pixel pairs are pooled from all training images (log domain) before fitting.
...
def train_ppn2v_multi(
    ...
    """
    Train shared PN2VUNet with neg. log-evidence loss on multiple log-domain images.
    ...
    print(f"Training on {len(train_ds.images)} image(s) (log domain)")
...
def _mmse_from_samples(
    ...
    """MMSE and prior mean from K network samples for one tile (log domain)."""
    K, H, W = pred_K.shape
    N       = H * W
...
def predict_tiled(
    ...
    Returns: (mmse_image, prior_image), both (H, W), in log domain.
    """
    if device is None:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Log + PPN2V juglab-faithful multi-image: log1p pre-transform + shared GMM + "
            "bootstrap N2V + K=800 UNet + per-image MMSE inference + expm1 inverse transform."
        )
    )
    ...
    if args.load_model and os.path.isfile(args.load_model):
        ...
        # ── 3a. Load training images and apply log transform ─────────────────
        print("\nLoading training images and applying log transform ...")
        log_train_images = []
        for p in train_paths:
            img, img_min, img_max = load_sem_image(str(p))
            log_img, log_min, log_max = apply_log_transform(img)
            log_train_images.append(log_img)
        ...
        # ── 3b. Phase 1: calibrate GMM (log domain) ──────────────────────────
        if args.calib_dir:
            ...
            print(f"\n[Phase 1] Bootstrap mode: running N2V for "
                  f"{args.n2v_epochs} epochs on {len(log_train_images)} image(s) ...")
            ...
            y_all = [img.flatten() for img in log_train_images]
            s_pairs = np.concatenate(s_all).astype(np.float32)
            y_pairs = np.concatenate(y_all).astype(np.float32)
            print(f"Collected {len(s_pairs):,} pixel pairs from "
                  f"{len(log_train_images)} image(s)")
    ...
    # ── 4. Load inference images with per-image log transform ───────────────
    print("\nLoading inference images and applying log transform ...")
    infer_log_images, infer_orig_images, infer_meta = [], [], []
    for p in infer_paths:
        img, img_min, img_max = load_sem_image(str(p))
        log_img, log_min, log_max = apply_log_transform(img)
        infer_orig_images.append(img)
        infer_log_images.append(log_img)
        infer_meta.append((img_min, img_max, log_min, log_max))
    ...
    noise_model.plot(save_path=str(out_dir / 'gmm_log_ppn2v_multi.png'))
    ...
    for i, (p, log_img, orig_img, meta) in enumerate(
            zip(infer_paths, infer_log_images, infer_orig_images, infer_meta)):
        img_min, img_max, log_min, log_max = meta
        print(f"\n[{i+1}/{len(infer_paths)}] {p.name}")

        mmse_log, prior_log = predict_tiled(
            model, log_img, noise_model,
            tile_size=tile_size,
        ...
        # Inverse log transform → linear [0,1] domain
        mmse_linear  = inverse_log_transform(mmse_log,  log_min, log_max)
        prior_linear = inverse_log_transform(prior_log, log_min, log_max)

        stem       = p.stem
        tif_path   = str(out_dir / f"{stem}_denoised_log_ppn2v_juglab.tif")
        prior_path = str(out_dir / f"{stem}_prior_log_ppn2v_juglab.tif")
        png_path   = str(out_dir / f"{stem}_comparison_log_ppn2v_juglab.png")
        save_outputs(orig_img, mmse_linear, prior_linear, img_min, img_max,
