# ============================================================
# SEM Image Denoising — Log + N2V with Parametric GMM + BIC Component Selection
# ============================================================
#
# Extends denoise_N2V_GMM_bic.py with a log1p pre-transform.
#
# Rationale:
...
#   capturing structure that plain log+N2V misses.
#
# Pipeline:
#   load_sem_image()
#     → apply_log_transform()           [log1p, renorm to [0,1]]
#       → BIC component selection       [on log-domain pixel pairs]
#       → GMM pre-training              [on log-domain (s_proxy, y_log) pairs]
#       → joint N2V + GMM training      [log domain]
#       → tiled inference               [log domain → denoised_log]
#     → inverse_log_transform()         [denorm, expm1 → linear [0,1]]
#       → restore original range → save TIF / PNG
#
# Position in script family:
# ──────────────────────────────────────────────────────────────────────────────
# Script                         │ Domain │ Noise model    │ Output  │ Inference
# ───────────────────────────────┼────────┼────────────────┼─────────┼──────────
# denoise_N2V_GMM_bic.py         │ linear │ GMM + BIC      │ scalar  │ raw UNet
# denoise_log_N2V_GMM_bic.py ←  │ log    │ GMM + BIC      │ scalar  │ raw UNet
# denoise_log_PPN2V_juglab.py    │ log    │ GMM (manual C) │ K=800   │ MMSE
# denoise_PPN2V_juglab_bic.py    │ linear │ GMM + BIC      │ K=800   │ MMSE
#
# When to use this script:
#   - Speckle-dominant + Poisson mixed SEM noise (ENL < 3)
#   - Faster than log_PPN2V_juglab (no bootstrap, no K=800 MMSE)
#   - Want auto n_components without manual trial
#   - When denoise_N2V_GMM_bic.py leaves residual speckle structure
#
...
#   python denoise_log_N2V_GMM_bic.py         # BIC selects n_components, trains + denoises
#   python denoise_log_N2V_GMM_bic.py --gmm_candidates 2 3 5
#   python denoise_log_N2V_GMM_bic.py --n_gaussians 3   # skip BIC
# ============================================================
...
class GMMNoiseModel(nn.Module):
    ...
    For each of n_gaussians components:
        μ_k(s)  = s + offset_k
...
# ============================================================
# 5. BIC-Based GMM Component Count Selection  (A3 — novel engineering addition)
# ============================================================
...
def select_num_gaussians_bic(
    ...
    """
    Select the number of GMM components using the Bayesian Information Criterion.

    BIC = n_params · ln(n) − 2 · ln(L̂)

    where n_params = 5·n_comp (weights n_comp-1, mean_offsets n_comp, var_a n_comp,
    var_b n_comp; one weight is constrained by normalization), and L̂ is the maximum
    likelihood of a sklearn GaussianMixture fitted on the 2D (s_proxy, y) feature space.

    The 2D joint GMM is a proxy for the conditional p(y|s) used in PN2V.
    It ranks component count candidates in the same order as the conditional model for
...
def pretrain_gmm(
    ...
    print(f"Pre-training GMM (n_components={noise_model.n_gaussians}, {n_epochs} epochs, {N:,} pairs) ...")
