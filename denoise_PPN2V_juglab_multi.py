# ============================================================
# SEM Image Denoising — Fully Unsupervised PPN2V, Multi-Image (juglab-faithful)
# ============================================================
# Based on: Krull et al., "Fully Unsupervised Probabilistic Noise2Void" (2020)
#   arXiv:1911.12291
#   GitHub: github.com/juglab/pn2v
#
# Extends denoise_PPN2V_juglab.py to train ONE shared model on multiple images
# acquired under similar SEM conditions, then denoise each image with MMSE.
#
# Multi-image advantages over single-image PPN2V:
#   + Bootstrap N2V trains on all images → more stable pseudo-clean reference
#   + GMM fitted from N × H × W pooled pixel pairs → richer noise statistics
#   + Shared PN2V UNet trains on a larger patch pool → more stable convergence
#
# Key design:
#   Noise model  │ Parametric signal-dependent GMM (n_components, default 3)
#                │ p(y|s) = Σ α_k(s)·N(μ_k(s), σ²_k(s)); α_k(s) = softmax(a_k·s + b_k)
#                │ GMM fitted from pooled pseudo-clean + noisy pairs; then FROZEN
#   UNet output  │ K=800 samples per pixel (K output channels)
#   Training     │ Negative log-evidence: -log(1/K Σ_k p_GMM(y|s_k))
#   Inference    │ Per-image MMSE: Σ p_GMM(y|s_k)·s_k / Σ p_GMM(y|s_k)
#
# When NOT to use:
#   Images acquired under substantially different SEM conditions (beam energy,
#   magnification, dose). A shared GMM averages noise statistics incorrectly
#   across conditions — use denoise_PPN2V_juglab.py per image instead.
#
# CLI (mirrors denoise_PN2V_juglab_multi.py):
#   python denoise_PPN2V_juglab_multi.py --input_dir ./sem_images --output_dir ./denoised
#   python denoise_PPN2V_juglab_multi.py --train_dir ./ref_imgs --input_dir ./targets --output_dir ./out
#   python denoise_PPN2V_juglab_multi.py --input_dir ./sem_images --save_model ppn2v.pt
#   python denoise_PPN2V_juglab_multi.py --input_dir ./new_imgs   --load_model ppn2v.pt
#
...
# ============================================================
# 6. Bootstrap N2V — Phase 1 (multi-image)
# ============================================================
...
# ============================================================
# 8. PPN2V Training — Phase 2 (multi-image, negative log-evidence)
# ============================================================
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "PPN2V juglab-faithful multi-image: shared GMM + bootstrap N2V + "
            "K=800 UNet + per-image MMSE inference."
        )
    )
    ...
    noise_model.plot(save_path=str(out_dir / 'gmm_ppn2v_multi.png'))
    ...
    for i, (p, img, (img_min, img_max)) in enumerate(
        ...
        stem       = p.stem
        tif_path   = str(out_dir / f"{stem}_denoised_ppn2v_juglab.tif")
        prior_path = str(out_dir / f"{stem}_prior_ppn2v_juglab.tif")
        png_path   = str(out_dir / f"{stem}_comparison_ppn2v_juglab.png")
