# ============================================================
# SEM Image Denoising — Noise2Void (PyTorch + Intel MKL CPU)
# ============================================================
# 策略三：純 CPU + Intel MKL-DNN（oneDNN）最佳化
#
# 適用情境：
#   - 無 NVIDIA GPU，只有 Intel 內建顯示卡（iGPU）
#   - Intel iGPU 為舊款（UHD 620/630），DirectML 加速效益低於 CPU
#   - torch-directml 安裝失敗或出現算子不支援的錯誤
#   - 追求穩定性優先於效能
#
# Requirements: torch  tifffile  matplotlib  numpy
#   （以上為標準 CPU 版 PyTorch，無需額外安裝 GPU 套件）
#
# 選用性加速（安裝後自動啟用）：
...
#   python denoise_torch_intel_mkl.py   # train + denoise -> denoised_sem_intel_mkl.tif
#
# MKL 啟用確認：
#   python -c "import torch; print(torch.__config__.show())"
#   # 應出現 USE_MKL=ON 及 USE_MKLDNN=ON
# ============================================================

import os
# 讓 PyTorch 的 MKL-DNN 自行管理執行緒，避免與 OMP 競爭
# 若手動設定 num_threads，須在 import torch 之前設定 OMP_NUM_THREADS
...
# ============================================================
# Device Setup — CPU + Intel MKL / IPEX
# ============================================================

def setup_cpu_device() -> torch.device:
    """
    設定 CPU 執行環境：
      1. 自動偵測實體核心數並設定 PyTorch 執行緒數
      2. 嘗試載入 Intel Extension for PyTorch（IPEX）
      3. 確認 MKL-DNN 是否啟用

    回傳 torch.device('cpu')。
    """
    # 實體核心數（非超執行緒），上限 16 避免過度分配
    physical_cores = min(multiprocessing.cpu_count(), 16)
    torch.set_num_threads(physical_cores)
    torch.set_num_interop_threads(2)   # 算子間並行，保持低值避免競爭

    print(f"[Device] CPU  |  threads={physical_cores}")

    # MKL-DNN 狀態確認
    mkl_enabled = torch.backends.mkl.is_available()
    mkldnn_enabled = torch.backends.mkldnn.is_available()
    print(f"[Device] MKL={mkl_enabled}  MKL-DNN(oneDNN)={mkldnn_enabled}")

    # 嘗試載入 IPEX（選用）
    try:
        import intel_extension_for_pytorch as ipex  # noqa: F401
        print("[Device] Intel Extension for PyTorch (IPEX) 已載入，訓練時將自動套用算子最佳化")
    except ImportError:
        print("[Device] IPEX 未安裝（選用）。"
              "執行 'pip install intel-extension-for-pytorch' 可進一步提升 Intel CPU 效能")

    return torch.device('cpu')


def apply_ipex(model: nn.Module, optimizer=None):
    """
    若 IPEX 可用，對模型（與 optimizer）套用 Intel 算子融合最佳化。
    若 IPEX 未安裝，原樣回傳，不影響執行。
    """
    try:
...
def train_n2v(
    ...
    image: np.ndarray,
    patch_size:     int   = 64,
    batch_size:     int   = 32,    # CPU 建議較小 batch，減少記憶體搬移
    num_epochs:     int   = 100,
    ...
    """
    Self-supervised N2V training on a single image using CPU + Intel MKL.

    CPU 調校重點：
      - batch_size=32：減少每次前向傳播的記憶體需求
      - num_workers=0：Windows 多程序開銷高，單執行緒 DataLoader 更快
      - pin_memory=False：僅對 CUDA 有效
      - IPEX optimize：若已安裝，在 Adam 建立後立即套用算子融合
    """
    if device is None:
    ...
    # IPEX 最佳化（若已安裝）：對 Conv + BN 等算子進行融合，提升 Intel CPU 效能
    model, optimizer = apply_ipex(model, optimizer)
    ...
    with torch.no_grad():
        for r0 in row_starts:
            for c0 in col_starts:
                ...
                tile_t  = torch.from_numpy(tile_np).unsqueeze(0).unsqueeze(0)
                pred_np = model(tile_t).squeeze().numpy()
...
def save_outputs(
    ...
    axes[1].set_title('N2V Denoised (Intel MKL CPU)')
    axes[1].axis('off')
