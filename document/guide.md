...
### 問題六：擴大盲區（3×3 或 5×5）後細節大量消失

**情境**：處理空間相關 speckle 時，有時建議將 blind-spot 從預設的 1×1 擴大至 3×3 或 5×5，以迫使網路從更遠的像素預測中心值，從而打破 grain 的空間相關性。

**根本 trade-off**：Blind-spot 越大，網路預測中心像素時的「視野起點」越遠，對於 SEM 這類用於觀測奈米結構的儀器，1–2 像素寬的微細結構（奈米碳管邊緣、量子點、晶格條紋）會被不可逆地抹平。

**使用前的量化檢查：**
```python
# 估算影像中最小關鍵特徵的寬度（需目視確認後手動輸入）
min_feature_px = 3   # 你的影像中最細結構的估計像素寬度

max_safe_blindspot = min_feature_px // 2
print(f"安全盲區上限：{max_safe_blindspot}px（超過此值將抹平最細特徵）")

# 若最細特徵 < 4px，3×3 盲區已超出安全範圍
if min_feature_px < 4:
    print("警告：最細特徵 < 4px，不建議使用 3×3 以上盲區")
    print("替代方案：AP-BSN（pixel-shuffle）或 N2V2（無需擴大盲區）")
```

**替代方案優先順序：**
- **AP-BSN**：以 pixel-shuffle 降採樣取代盲區擴大，處理空間相關噪聲時對高頻細節的破壞顯著更小
- **N2V2**：改良了 MaxPool 與 skip connection，消除棋盤格假影，通常不需要擴大盲區
- **Self2Self / GR2R**：不依賴 blind-spot 機制，從根本上避免此問題
