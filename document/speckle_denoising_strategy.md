...
**延伸：GAT + PN2V 的數學自洽性問題**

廣義 Anscombe 轉換（GAT）的設計目標是將 Poisson-Gaussian 混合噪聲的「訊號相依性」完全抹平，轉化為方差接近 1 的加性高斯白噪聲（AWGN）。若 GAT 轉換完全成功，殘餘噪聲應服從高斯分佈，此時使用普通 N2V 配合 L2 loss 即為數學最優解，不需要 PN2V。PN2V 的優勢在於能學習「複雜、非高斯、訊號相依」的噪聲模型——但這正是 GAT 已試圖消除的特性。

兩者疊加使用時的實際意義：PN2V 的 GMM 主要在對 **GAT 的殘餘轉換誤差**建模，而非原始噪聲。在低訊號區，GAT 的方差穩定化效果本就不完美（見 §4），此時 PN2V 的建模能力確有補救空間；但在高訊號區，組合反而可能使 GMM 過度擬合 GAT 轉換誤差而非真正的物理噪聲。

**實踐建議：** GAT 後優先嘗試 N2V，僅在目視觀察到低訊號區仍有明顯假影時，才考慮改用 PN2V，並同時比較是否比直接在原始空間訓練 PN2V（不做 GAT）效果更好。
...
**空間相關長度臨界條件：** S2S 能有效去噪的前提是：Dropout 遮蔽的範圍超過 speckle grain 的空間相關長度，使網路無法從鄰近未遮蔽像素重建 grain 殘影。若 SEM 放大倍率極高，導致 grain 橫跨 3×3 甚至 5×5 像素，即使 30% Dropout 也無法阻斷 grain 訊息的洩漏，S2S 可能將 speckle grain 誤判為真實訊號紋理而保留下來。

**先診斷 grain size，再決定方法：**

```python
from scipy.signal import fftconvolve
import numpy as np

def estimate_speckle_grain_size(image):
    """估計 speckle grain 的空間相關長度（像素）"""
    img = image - image.mean()
    acf = fftconvolve(img, img[::-1, ::-1], mode='full')
    acf = acf / acf.max()
    center_row = acf.shape[0] // 2
    center_col = acf.shape[1] // 2
    acf_row = acf[center_row, center_col:]
    below_threshold = np.where(acf_row < 1 / np.e)[0]
    grain_size = int(below_threshold[0]) if len(below_threshold) > 0 else 1
    return grain_size

grain_px = estimate_speckle_grain_size(image)
print(f"估計 grain size: {grain_px} 像素")
if grain_px > 2:
    print("grain > 2px：Self2Self 效果降低，建議改用 GR2R")
else:
    print("grain ≤ 2px：Self2Self 適用")
```

若 `grain_px > 2`，應優先選用 **GR2R**（見方案 D），而非強行套用 Self2Self。
...
> ⚠️ **極低計數邊界條件（Extreme Low-dose 警告）**
>
> `log1p` 與 `+ 1e-8` 僅防止數學崩潰，**無法解決極低計數下的偏差問題**。當 SEM 處於極低電子劑量時，像素計數可能在 0–3 之間跳動，此時：
> - 暗區計數的微小擾動（0 → 1）在 log 域造成非線性放大（log(1+1)≈0.69 vs log(1+0)=0）
> - 逆轉換後的黑背景區域容易出現灰化偏差或邊緣假影
>
> **診斷方法：** 計算影像的同質背景區域均值。若歸一化後均值低於 0.02，或原始像素值估計計數 < 5，應視為低計數場景。
>
> ```python
> background_mean = image[np.s_[10:30, 10:30]].mean()
> if background_mean < 0.02:
>     print("警告：極低計數場景，log 轉換偏差風險高")
>     print("建議：改用 PN2V 或 GR2R 在原始強度空間直接訓練")
> ```
>
> **處置原則：** 低計數場景應放棄前處理轉換策略，優先選用能在原始 Poisson 計數空間建模的方法（PN2V 或 GR2R），讓模型直接學習訊號相依的噪聲特性。
...
N2V 或 PN2V 在第一張影像訓練後重複套用給後續影像，在自然攝影場景下通常安全，但在 SEM 產線環境中風險較高。以下因素可能使後續影像的噪聲分佈與第一張產生系統性偏移：

- **樣品帶電效應（Charging effect）**：非導電樣品在長時間電子束照射後電荷積累，影響二次電子訊號強度
- **電子束電流漂移（Beam current drift）**：連續掃描中探針電流緩慢衰退，整體亮度降低
- **散焦或樣品高度偏移**：影像模糊程度變化，改變有效空間頻率噪聲結構

**診斷方法：** 比較第一張與最後一張影像的背景亮度均值與標準差差異。

```python
def check_batch_stability(images):
    """比較批次影像的統計一致性"""
    ref_mean = images[0].mean()
    ref_std  = images[0].std()
    for i, img in enumerate(images[1:], 1):
        mean_shift = abs(img.mean() - ref_mean) / ref_mean
        std_shift  = abs(img.std()  - ref_std)  / ref_std
        if mean_shift > 0.1 or std_shift > 0.15:
            print(f"影像 {i}：亮度偏移 {mean_shift:.1%}，噪聲強度偏移 {std_shift:.1%} → 建議重新訓練或校正")

check_batch_stability(image_list)
```

若偏移超過 10%，需在套用共用模型前做**全域直方圖匹配**（histogram matching），將後續影像的統計分佈對齊至訓練影像，或對每張影像分別訓練。

```python
from skimage.exposure import match_histograms

# 將 target 影像匹配至 reference 影像的直方圖
aligned = match_histograms(target_image, reference_image)
```
